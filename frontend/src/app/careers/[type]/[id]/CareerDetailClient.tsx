...
import { getJobById, getInternshipById, applyForPosition, JobPosting, InternshipPosting } from '@/lib/server/api';
import { useAuth } from '@/context/AuthContext';

type PageType = 'jobs' | 'internships';

export default function CareerDetailPageClient({
  type,
  id,
  initialItem
}: {
  type: string;
  id: string;
  initialItem: JobPosting | InternshipPosting | null;
}) {
  ...
  const [item, setItem] = useState<JobPosting | InternshipPosting | null>(initialItem);
  ...
  // Fetch job/internship details if they change or are not loaded yet
  useEffect(() => {
    if (initialItem && item?._id === id) {
      setIsLoading(false);
      return;
    }
          ...
          <div className="w-full max-w-md rounded-xl bg-white p-8 shadow-2xl">
            <div className="text-center">
              ...
              <div className="flex flex-col gap-3">
                ...
                <Link
                  href="/"
                  className="rounded-lg border-2 border-gray-300 px-6 py-3 text-center font-semibold text-[#5f6368] transition-all hover:border-gray-400 hover:bg-gray-50"
                >
                  Back to Home
                </Link>
              </div>
