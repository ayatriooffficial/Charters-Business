  ...
  if (!applications || applications.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 px-4">
        ...
        <Link href="/apply" className="flex items-center gap-2 bg-[#B30437] hover:bg-[#8B0329] text-white font-semibold py-3 px-6 rounded-lg transition-colors">
          Apply Now
        </Link>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto px-4">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-gray-600">You have {applications.length} application{applications.length > 1 ? 's' : ''}</p>
      </div>
      {applications.map((application: any, index: number) => (
        <article key={application.applicationNumber || index} className="flex flex-col bg-white border border-gray-200 overflow-hidden">
          <div className="flex flex-col p-4 sm:p-6 gap-6">
            <div className="flex flex-col gap-1 pb-4 border-b border-gray-200">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900">{application.program}</h2>
              <p className="text-sm text-[#5f6368]">Application #{application.applicationNumber}</p>
            </div>
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                <div className="flex flex-row justify-between items-center py-2 border-b border-gray-100">
                  ...
                  <span className="text-sm font-semibold text-gray-900">{application.applicationNumber}</span>
                </div>
                <div className="flex flex-row justify-between items-center py-2 border-b border-gray-100">
                  <span className="text-sm text-gray-600">Program</span>
                  <span className="text-sm font-semibold text-gray-900 text-right">{application.program}</span>
                </div>
                <div className="flex flex-row justify-between items-center py-2">
                  <span className="text-sm text-gray-600">Submitted On</span>
                  <span className="text-sm font-semibold text-gray-900">{formatDate(application.createdAt)}</span>
                </div>
              </div>
            </div>
          </div>
...
export default function ApplicationStatusPage() {
  ...
  useEffect(() => {
    navigateToRemoteDashboard('/application-status');
