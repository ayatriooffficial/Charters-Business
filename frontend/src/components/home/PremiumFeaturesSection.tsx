...
const PremiumFeaturesSection = () => {
  ...
  useEffect(() => {
    ...
    const progressInterval = setInterval(() => {
      setProgress((prev) => (prev >= 100 ? 100 : prev + 100 / 40));
    }, 100);
  ...
  const renderFeatureContent = () => {
    if (activeFeature === "ai-instruction") {
      return (
        <div className="bg-white overflow-hidden" role="complementary" aria-labelledby="ai-demo-heading">
          <div className="p-6">
            <h3 id="ai-demo-heading" className="sr-only">AI Instruction Demo</h3>
            <div className="bg-gradient-to-br from-purple-100 to-blue-100 h-64 flex items-center justify-center">
              <div className="text-center">
                ...
                <h4 className="text-lg font-semibold text-black mb-2">AI-Powered Learning</h4>
                <p className="text-black">Personalized instruction adapts to each student&apos;s needs</p>
              </div>
            </div>
          </div>
        </div>
    ...
    if (activeFeature === "reading-skills") {
      return (
        <div className="bg-white overflow-hidden" role="complementary" aria-labelledby="reading-demo-heading">
          <div className="p-6">
            <h3 id="reading-demo-heading" className="sr-only">Reading Skills Demo</h3>
            <div className="bg-gradient-to-br from-green-100 to-teal-100 h-64 flex items-center justify-center">
              <div className="text-center">
                <div className="w-16 h-16 bg-[#B30437] rounded-full mx-auto mb-4 flex items-center justify-center">
                  <span className="text-white text-2xl">📚</span>
                </div>
                <h4 className="text-lg font-semibold text-black mb-2">Reading practice with fun books</h4>
                <p className="text-black">Build independent reading skills through engaging content</p>
              </div>
            </div>
          </div>
        </div>
    ...
    if (activeFeature === "self-paced") {
      return (
        <div className="bg-white overflow-hidden" role="complementary" aria-labelledby="demo-heading">
          <div className="bg-gray-100 px-4 py-3">
            <div className="flex items-center space-x-2">
              <button className="w-3 h-3 bg-red-400 rounded-full" aria-label="Close window" />
              <button className="w-3 h-3 bg-yellow-400 rounded-full" aria-label="Minimize window" />
              <button className="w-3 h-3 bg-green-400 rounded-full" aria-label="Maximize window" />
            </div>
          ...
          <div className="p-6">
            <h3 id="demo-heading" className="sr-only">Interactive Learning Demo</h3>
            <div
              ...
              role="img"
              aria-label="Educational video about photosynthesis"
            >
                  ...
                  <div className="absolute inset-0">
                    {[...Array(8)].map((_, i) => (
                      <div
                        ...
                        style={{ top: "-8px", left: "50%", transformOrigin: "50% 40px", transform: `translateX(-50%) rotate(${i * 45}deg)` }}
                      />
                    ))}
                  </div>
                </div>
              ...
              <div className="flex space-x-8">
                <div className="relative">
                  <div className="w-12 h-12 relative">
                    {[...Array(8)].map((_, i) => (
                      <div key={i} className="absolute w-3 h-6 bg-pink-400 rounded-full" style={{ top: "50%", left: "50%", transformOrigin: "50% 100%", transform: `translate(-50%, -100%) rotate(${i * 45}deg)` }} />
                    ))}
                ...
                <div className="relative">
                  <div className="w-8 h-8 relative">
                    {[...Array(6)].map((_, i) => (
                      <div key={i} className="absolute w-2 h-4 bg-pink-300 rounded-full" style={{ top: "50%", left: "50%", transformOrigin: "50% 100%", transform: `translate(-50%, -100%) rotate(${i * 60}deg)` }} />
                    ))}
            ...
            <div className="space-y-4">
              <h4 className="font-semibold text-black">Which organisms can perform photosynthesis?</h4>
              <fieldset className="space-y-3">
                ...
                {[
                  ...
                  { id: "bacteria", label: "Bacteria", value: "bacteria" },
                ].map((option) => (
                  <label key={option.id} className="flex items-center space-x-3 cursor-pointer group" htmlFor={option.id}>
                    <input
                      type="radio"
                      id={option.id}
                    ...
                    />
                    <span className={`text-sm ${selectedAnswer === option.value ? "text-black font-medium" : "text-gray-700"} group-hover:text-black transition-colors`}>
                      {option.label}
                    </span>
                    {selectedAnswer === option.value && option.value === "bacteria" && (
                      <div className="ml-auto w-5 h-5 bg-[#B30437] rounded-full flex items-center justify-center" role="img" aria-label="Correct answer">
              ...
              <div className="flex justify-between items-center mt-6 pt-4">
                <div className="flex space-x-1">
                  {[...Array(5)].map((_, i) => (
                    <div key={i} className={`w-2 h-2 rounded-full ${i < 3 ? "bg-[#B30437]" : "bg-gray-300"}`} role="img" aria-label={`Progress step ${i + 1}`} />
                  ))}
                </div>
    ...
    if (activeFeature === "integrations") {
      return (
        <div className="bg-white rounded-lg shadow-lg overflow-hidden" role="complementary" aria-labelledby="integrations-demo-heading">
          <div className="p-6">
            <h3 id="integrations-demo-heading" className="sr-only">Integrations Demo</h3>
            <div className="bg-gradient-to-br from-orange-100 to-red-100 rounded-lg h-64 flex items-center justify-center">
              <div className="text-center">
                <div className="w-16 h-16 bg-[#B30437] rounded-full mx-auto mb-4 flex items-center justify-center">
                  <span className="text-white text-2xl">🔗</span>
                </div>
                <h4 className="text-lg font-semibold text-black mb-2">Popular Integrations</h4>
                <p className="text-black">Connect with tools your students already use</p>
              </div>
            </div>
          </div>
        </div>
  ...
  /*
    ProgressBar — the fix for the non-composited animation Lighthouse flags.

    BEFORE: style={{ height: `${progress}%` }}
    Animating `height` forces the browser to recalculate layout on every frame
    because it changes the box model. This is the "Unsupported CSS Property: height"
    warning in the Lighthouse non-composited animations audit.

    AFTER: transform: scaleY(progress / 100) + transform-origin: top
    scaleY runs entirely on the GPU compositor thread — zero layout recalculation.
    The bar visually grows from top to bottom identically to the height approach.
    The container keeps h-full so it always occupies the full available height,
    and the bar scales within that space.
  */
  const ProgressBar = () => (
    <div
      className="absolute left-0 top-0 w-1 h-full bg-[#B30437] rounded-r origin-top"
      style={{
  ...
  const FeatureItem = ({
    ...
    const titleId = `feature-${isMobile ? "mobile-" : ""}${feature.id}-title`;
    ...
    return (
      ...
      >
        ...
        <h3
          id={titleId}
          className={`${isMobile ? "text-base" : "text-lg"} font-semibold mb-2 transition-colors duration-500 ${isActive ? "text-black" : "text-gray-700"}`}
        >
          {feature.title}
        </h3>
        ...
        {isActive && (
          <div className="animate-fadeIn">
            <p className={`${isMobile ? "text-sm" : ""} text-black leading-relaxed mb-3`}>
              {feature.description}
            </p>
            <button
      ...
      <div className="max-w-7xl w-full px-2 md:mx-auto">
        ...
        <div className="lg:hidden">
          ...
          <div className="space-y-6" role="list" aria-label="Premium learning features">
            {features.map((feature) => (
              <div key={feature.id} className="border-b border-gray-200 pb-6" role="listitem">
                <FeatureItem feature={feature} isMobile />
                {feature.id === activeFeature && (
                  <div className="mt-4 animate-fadeIn">{renderFeatureContent()}</div>
                )}
              </div>
            ))}
          </div>
