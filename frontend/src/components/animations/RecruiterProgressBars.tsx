...
const RecruiterProgressBars: React.FC<RecruiterProgressBarsProps> = ({
  ...
  const getBarPercentage = useCallback(
    (val: number) => (val / scaleInfo.maxValue) * 100,
    [scaleInfo.maxValue],
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-end mt-6">
        <div className="w-full">
          {recruiterData.map((item) => (
            <div
              ...
              style={{
                /*
                  BEFORE: transition: "all 0.6s ease-out"
                  `all` captures every CSS property including non-composited ones
                  like color, border, font-size — triggering layout recalculation.
  
                  AFTER: explicit opacity + transform only.
                  Both are GPU-composited and never touch the layout engine.
                  Removed willChange — premature promotion wastes GPU layers.
                */
            ...
            >
              <span className="text-[8px] sm:text-[9px] md:text-[10px] lg:text-[11px] text-[#80868b] font-light w-6 sm:w-7 text-right flex-shrink-0">
                {item.year}
              </span>

              <div
                className="flex-1 bg-transparent h-10 sm:h-14 md:h-16 relative border-l min-w-0 flex items-center"
                role="progressbar"
              ...
              >
                <div
                  className="bg-[#F3D8C5] h-10 sm:h-12 md:h-14 transition-[width] duration-1000 ease-out"
                  style={{
              ...
              <span
                ...
                style={{
                  /*
                    Same fix as the row container above — explicit properties only.
                    Removed willChange for the same reason.
                  */
                  opacity:
              ...
              <span className="text-[8px] sm:text-[9px] md:text-[10px] lg:text-[11px] text-[#80868b] w-8 sm:w-9 md:w-10 font-light truncate flex-shrink-0">
                {item.label}
              </span>
            </div>
          ))}
        ...
        <div className="mt-3 sm:mt-4 border-t border-gray-800 pt-2 flex-shrink-0 w-full">
          <div
            className="flex justify-between text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] text-[#5f6368] mb-1 ml-7 sm:ml-9 pr-8 sm:pr-9 md:pr-10"
            role="img"
            aria-label="Scale markers"
          >
            {scaleInfo.markers.map((marker, index) => (
              <span key={index} className="flex-shrink-0">{marker}</span>
            ))}
          </div>
          <div className="text-center text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] text-[#5f6368]">
            No. of Recruiters
          </div>
        </div>
      </div>
